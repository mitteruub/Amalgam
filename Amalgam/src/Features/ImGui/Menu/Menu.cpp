			...
			TableNextColumn();
			{
				if (Section("Colors", 8))
				{
					...
					if (FGet(Vars::Colors::Relative))
					{
						...
						FColorPicker(Vars::Colors::EnemyName, 0, FColorPickerEnum::Middle);
						FColorPicker(Vars::Colors::Team, 0, FColorPickerEnum::Left);
						FColorPicker(Vars::Colors::TeamName, 0, FColorPickerEnum::Middle);
					}
					else
					{
						FColorPicker(Vars::Colors::TeamRed, 0, FColorPickerEnum::Left);
						FColorPicker(Vars::Colors::TeamRedName, 0, FColorPickerEnum::Middle);
						FColorPicker(Vars::Colors::TeamBlu, 0, FColorPickerEnum::Left);
						FColorPicker(Vars::Colors::TeamBluName, 0, FColorPickerEnum::Middle);
					}
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (auto it = F::Binds.m_vBinds.begin(); it < F::Binds.m_vBinds.end(); it++)
			{
				...
				if (tBind.m_iVisibility == BindVisibilityEnum::Always || tBind.m_iVisibility == BindVisibilityEnum::WhileActive && tBind.m_bActive || m_bIsOpen)
				{
					...
					else if (tBind.m_pVar->m_iType == typeid(int).hash_code() && !(tBind.m_pVar->m_iFlags & DROPDOWN_MULTI))
					{
